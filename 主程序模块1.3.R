...
#     本脚本与主程序模块1.1和1.2的不同之处在于：数据为秒钟数据，入格模型为先秒入
#   格再按分钟汇总
# -----------------------------------------------
...
f.gridJoin.estimate <- function(datals,type.num = 0.6035534){
  d.result <- as.data.frame(matrix(0 , 0 , 3))
  colnames(d.result) <- c("DWMac_TID","grid_id","datetime_minute")

  for(index in unique(datals$WP_Mac)){
    datals2<-datals[datals$WP_Mac==index,]
    d.equal <- as.data.frame(matrix(0 , 0 , 3))
    colnames(d.equal) <- c("DWMac_TID","grid_id","datetime_minute")
    for(i in 1:dim(datals2)[1]){
      ...
      if(length(df)){
        df <- data.frame(rep(unique(datals$DWMac_TID),length(df)),df,rep(unique(datals$datetime_minute),length(df)))
        colnames(df) <- c("DWMac_TID","grid_id","datetime_minute")
  ...
  if(dim(d.result)[1]>=1){
  #先定义频数统计表
  freq.table <- plyr::count(d.result , vars = c("grid_id","datetime_minute"))
  #选取频数超过3的格子，认为每个格子权重相等的情况下再统计频数
  d.freq.table <- freq.table[freq.table$freq >= 3,]
  d.freq.table$grid_id <- as.character(d.freq.table$grid_id)
  d.freq.table$datetime_minute <- as.character(d.freq.table$datetime_minute)
  freq.table <- plyr::count(d.freq.table[,c(1,2)],vars = "grid_id")
  max.nums <- max(freq.table$freq)
  
  #返回结果
  d.equal <- d.result[d.result$grid_id  %in%  freq.table[freq.table$freq ==max.nums,]$grid_id,]
  zz <- unique(d.equal[,c("DWMac_TID","grid_id")])
  zz$grid_id <- as.character(zz$grid_id)
  zz$DWMac_TID <- as.character(zz$DWMac_TID)
  }else{
    zz <- data.frame("DWMac_TID"=as.character(datals$DWMac_TID),"grid_id"=NA)
  }
  return(zz)
} 
...
F.grid.join <- function(d.sl,type.num.initial = 0.6035534){
  ...
  result <- result[!is.na(result$grid_id),]
...
F.grid.loop <- function(D.data.for.estimate = D.merge.wifi.data,type.num.initial,
  ...
  d.sl$DWMac_TID <- apply(d.sl,1,function(x) paste(substr(x["DWMac_TID"],1,24),"00",sep = ""))
  #输出结果
